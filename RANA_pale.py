...
def parse_args():
    ...
    parser.add_argument('--prefix1',             default="../graph_data/Facebook_Twitter/Facebook/graphsage/")
    parser.add_argument('--prefix2',             default="../graph_data/Facebook_Twitter/Twitter/graphsage/")    
    parser.add_argument('--dataset',             default='Facebook_Twitter')
    parser.add_argument('--groundtruth',        default="../graph_data/Facebook_Twitter/dictionaries/")
...
def begin_self_labeling():
    model = PALE(source_dataset, target_dataset, args, e_train_dict, device)
    S, S_normalized = model.align()
    acc, MAP, top5, top10, S_pairs = get_statistics(S, groundtruth, use_greedy_match=True, get_all_metric=True)
    print("Acc:", acc, MAP, top5, top10)
    model_r = acc
    model_belief_list = np.zeros(n1 * n2)
    self_label_pairs = list()
    for pair in available_pairs:
        a, b = pair
        bt = S_normalized[a][b] * model_r  # 如果模型对单个节点的预测*模型整体的预测准确率大于oracle_acc，就认为预测是可用的
...
def find_mirror_pair(i, j):
    ...
    i_prime = np.argmax(similarity_feature_g1[i, :])  # Find the most similar node to i in graph 1
    j_prime = np.argmax(similarity_feature_g2[j, :])  # Find the most similar node to j in graph 2
...
gt_reverse = { v:k for k, v in groundtruth.items()}
...
# avaliable_pairs = graph_utils.load_gt(os.path.join(args.groundtruth, f"node,split={args.rate}.test.dict"),
                                    # source_dataset.id2idx, target_dataset.id2idx, 'dict', args.dataset)  # 候选节点对
...
# feature_cos = cosine_similarity(feature_g1_cpu, feature_g2_cpu)
...
np.save("../graph_data/Facebook_Twitter/Confidence_matrix_fbtt_0.9.npy", init_Confidence_matrix)
'''
# Initialize Confidence score
# Confidence_matrix = np.ones((n1, n2)) * args.oracle_acc
Confidence_matrix = np.load("../graph_data/Facebook_Twitter/Confidence_matrix_fbtt_0.8.npy")
...
flag = np.zeros(n1 + n2)  # 被选择过的节点就不能再选择了，0表示没有被选择过
...
while True:
    ...
    if max_activated_num <= 0:
        break 
    train_list.append(max_influence_pair) 
    ...
    print("Effective Pairs:", pairs_label.count(1))
    ...
    if len(effective_train_list) >= (len(train_dict) + args.max_al_num):
        break
