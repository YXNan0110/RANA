...
def parse_args():
    ...
    parser.add_argument('--rate', default=0.05, type=float)  # training rate
    parser.add_argument('--oracle_acc', default=1, type=float)
    parser.add_argument('--AL_batch_size', default=10, type=int)
    parser.add_argument('--th', default=0.7, type=float)  # 相似度大于这个阈值就不作为available_pair
    parser.add_argument('--gamma', default=0.05, type=float)
    parser.add_argument('--theta', default=0.5, type=float)  # 可信度指标
    parser.add_argument('--max_al_num', default=50, type=int)
...
def begin_self_labeling():
    model = IsoRank(source_dataset, target_dataset, None, args.alpha, args.max_iter, args.tol, e_train_dict)
    S, S_normalized = model.align()  # 对齐概率矩阵
    # print(S)
...
np.save("../graph_data/douban/Confidence_matrix_douban_1.npy", init_Confidence_matrix)
'''
# Initialize Confidence score
# Confidence_matrix = np.ones((n1, n2)) * args.oracle_acc
Confidence_matrix = np.load("../graph_data/douban/Confidence_matrix_douban_1.npy")
